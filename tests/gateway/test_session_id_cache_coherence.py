...
def _guard_would_reuse(runner, session_key, session_id):
    ...
    """
    try:
        # Mirror the production guard site (gateway/run.py), which reads the
        # count synchronously off the wrapped raw SessionDB (``._db``).
        _raw_db = getattr(runner._session_db, "_db", runner._session_db)
        row = _raw_db.get_session(session_id)
