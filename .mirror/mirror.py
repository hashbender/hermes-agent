...
def main():
    ...
    log("== fetch (heads) ==")
    # Fetch branch heads only. We deliberately do NOT bulk-fetch
    # `+refs/pull/*/head` here: on a repo with tens of thousands of PRs that
    # pulls gigabytes to use a handful of refs (and the watcher would repeat it
    # every cycle). The specific PR heads we replay are fetched selectively
    # below, after the candidate list is known.
    ...
    # Selective PR-head fetch: only the candidates at/after the baseline (bounded
    # by `gh pr list --limit 100`). This populates refs/mirror-src/pr/<n> and the
    # ancestor commits build_overlay / first_commit_parent need.
    if prs:
        log("== fetch ({} PR heads >= #{}) ==".format(len(prs), SINCE_PR))
        git("fetch", "--quiet", ORIGIN,
            *["+refs/pull/{0}/head:refs/mirror-src/pr/{0}".format(p["number"]) for p in prs])
    all_prs = list(prs)
    bases = {}
    for p in prs:
