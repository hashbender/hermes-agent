...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    if agent._fallback_index >= len(agent._fallback_chain):
        ...
        # their own cooldown above), arm a short cooldown so the next turn's
        ...
        if (
            ...
            # Use a 1-failure mark so the manager uses its base duration, but
            # only if we're not already in a longer cooldown.  We pass a tiny
            # manager that won't collide with the singleton's state.
