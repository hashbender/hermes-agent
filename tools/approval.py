...
DANGEROUS_PATTERNS = [
    ...
    (r'\bchmod\s+(-[^\s]*\s+)*(777|666|o[+=][rwx]*w[rwx]*|a[+=][rwx]*w[rwx]*)\b', "world/other-writable permissions"),
    (r'\bchmod\s+--recursive\b.*(777|666|o[+=][rwx]*w[rwx]*|a[+=][rwx]*w[rwx]*)', "recursive world/other-writable (long flag)"),
    (r'\bchown\s+(?:-[^\s]+\s+)*?-[a-z]*r[a-z]*\s+(?:-[^\s]+\s+)*root\b', "recursive chown to root"),
    (r'\bchown\s+(?:-[^\s]+\s+)*?--recursive\s+(?:-[^\s]+\s+)*root\b', "recursive chown to root (long flag)"),
